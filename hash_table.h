...
*/
// size: number of rows in the hashtable
// MAKE SURE HASHTABLE IS OF SIZE POWER OF 2 MINUS 1, MAKE IT 63 OR 127-THAT IS WHAT WE WILL USUSALLY NEED
...
/* 
   ...
   note: a data element can be inserted more than once in this
		 ...
		 **ptR=28
		 HASH__INSERT(......,PTR);
		 dont have to do this for key, this function will handle memory allocation for key
		 for single pass:
		 use linked list as the data, in the first pass, if we have a froward address -when we find the "exit" go through the linked list for all the places we use exit
		 delete every elemnt in the hashtable, befrore program exits
*/
static inline int32_t hash_insert( hash_table_t *hash_table, void *key, uint32_t key_len, void *data)
{
  ...
  sem_post(&hash_table->row_lock[hash_key]);
#endif
  //printf("hash unit key: %s, value: %s\n\n", new_entry->data, new_entry-> key);//debug
  return(TRUE);
}
...
/*
  ...
  NULL on failure
  //make instr_ptr a global variable
  //never pass stack variables to his functions
*/
